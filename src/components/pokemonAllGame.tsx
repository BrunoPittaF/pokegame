...
const PokemonFirstGame: React.FC = () => {
  ...
  // const [isMediumMode, setIsMediumMode] = useState(true);
  ...
  // const applyEasyMode = async () => {
  //   setIsMediumMode((prev) => !prev);
  //   const allPokemonData: { [key: number]: any } = {};
  //   for (let i = 0; i <= 1025; i++) {
  //     try {
  //       allPokemonData[i] = pokemonList[i];
  //       allPokemonData[i].showName = false;
  //     } catch {
  //       console.error('caio');
  //     }
  //   }
  ...
  return (
    <div className="p-4 max-sm:p-0 min-h-screen flex flex-col items-center m">
      <h1 className="text-3xl font-bold text-center text-white-800">Adivinhe os pokemons!</h1>
      <p>Melhor Partida: {record}</p>
      <p>Pontuação: {pontuation > record ? `Novo Recorde! ${pontuation} pontos` : record}</p>
      {/* {!startedGame && <SwitchComponent isMedium={isMediumMode} onToggle={applyEasyMode} />} */}
